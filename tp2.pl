...
nfa_match(NFA,step(Steps),[],[Char|Chars],[],Tail) :-
    member((Char -> State),Steps),
    member((State = StepAfter),NFA),
    nfa_match(NFA,StepAfter,[],Chars,[],Tail).

nfa_match(NFA, step(Step), [], [_|Chars], [], Tail) :-
    member(Step = StepAfter,NFA),
    nfa_match(NFA,StepAfter, [], Chars, [], Tail).

nfa_match(NFA,epsilon([],[State|States]),[],[Char| Chars],[],Tail) :-
    member(State = EpsStep, NFA),
    nfa_match(NFA,EpsStep,[],[Char| Chars],[],Tail);
    nfa_match(NFA,epsilon([],States),[],[Char| Chars],[],Tail).

nfa_match(NFA,step(Steps),[],[Char|Chars],[],Tail) :-
    member((Char -> _), Steps) -> fail;
    append(Init, Last, Steps),
    Init \= [],
    state_wf(Last, NFA),
    member((Last = StepAfter),NFA),
    nfa_match(NFA, StepAfter, [], Chars, [], Tail).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% init_list(+List, -List)
%% copie la meme liste mais sans le dernier élément.
init_list(List, InitList) :-
    reverse(List, [_ | Y]),
    reverse(Y, InitList).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% new_state(-State)
...
re_comp(Char, End, Begin, NFA) :-
    character(Char),
    new_state(Begin),
    NFA = [Begin = step([Char->End])].

re_comp(in([]), _, _, _).
re_comp(in(String), End, Begin, NFA) :-
    string_chars(String, [Char| Chars]),
    re_comp(in(Chars), End, Mid, [Mid=step(NFA1)]),
    re_comp(Char, End, Begin, [Begin=step(NFA0)]),
    append(NFA0, NFA1, NFA2),
    init_list(NFA2, _),
    NFA = [Begin = step(NFA2)].

re_comp(notin([]), End, Begin, NFA) :-
    new_state(Begin),
    NFA = [Begin=step(End)].
re_comp(notin(String), End, Begin, NFA):-
    string_chars(String, [Char| Chars]),
    re_comp(notin(Chars), End, Begin, [Begin =step(NFA2)]),
    re_comp(Char, State, State, [State=step(NFA1)]),
    append(NFA1, NFA2, NFA0),
    NFA = [Begin = step(NFA0)].

re_comp(RE1 \/ RE2, End, Begin, NFA) :-
    new_state(Begin),
    re_comp(RE1, End, Begin1, NFA1),
    re_comp(RE2, End, Begin2, NFA2),
    append(NFA1, NFA2, NFA0),
    NFA = [Begin=epsilon([], [Begin1, Begin2]) | NFA0].

re_comp(?(RE), End, Begin, NFA):-
    new_state(Begin),
    re_comp(RE, End, Begin1, NFA1),
    NFA = [Begin= epsilon([],[Begin1, End])| NFA1].

re_comp(+(RE), End, Begin, NFA):-
    re_comp(*(RE), End, Begin1, NFA2),
    re_comp(RE, Begin1, Begin, NFA1),
    append(NFA1, NFA2, NFA).
